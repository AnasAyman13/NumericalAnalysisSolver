...
object LUSolver {
     ...
     * Rewritten to exactly match the university C++ LUDecomposition algorithm formulas for 3x3 matrices.
     */
    ...
    private fun solve3x3(aMatrix: Array<DoubleArray>, bVector: DoubleArray): LinearSystemResult {
        ...
        val p = intArrayOf(0, 1, 2) // Permutation tracking for Partial Pivoting (PA = LU)
        ...
        if (maxRow != 0) {
            ...
            steps.add(LinearStep("LU Pivot: Swapped Row 1 with Row ${maxRow+1}", copyAugmented(a)))
        }

        if (abs(a[0][0]) < 1e-12) throw SingularMatrixException("Pivot a[0][0] is zero after pivoting. System is singular.")

        // Output of GJE multipliers
        ...
        if (abs(a[2][1]) > abs(a[1][1])) {
            ...
            // Must swap multipliers in L matrix to maintain correct PA=LU mapping
            val tm = m21; m21 = m31; m31 = tm
            steps.add(LinearStep("LU Pivot: Swapped Row 2 with Row 3", copyAugmented(a)))
        }
        ...
        // Matrix U = result of GJE (excluding augmented b)
        val u = Array(3) { r -> DoubleArray(3) { c -> a[r][c] } }
        steps.add(LinearStep("U matrix", copyMatrix(u)))

        // Matrix L = Identity with multipliers
        val l = Array(3) { r -> DoubleArray(3) { c -> if (r == c) 1.0 else 0.0 } }
        l[1][0] = m21
        l[2][0] = m31
        l[2][1] = m32
        steps.add(LinearStep("L matrix", copyMatrix(l)))

        // Solve Lc = b
        // In the original without pivoting, `b` is unmodified.
        // With pivoting, we must apply the exact permutations `p` to `b` before solving.
        ...
        steps.add(LinearStep("Vector C (Result of Lc = b)", arrayOf(doubleArrayOf(c1, c2, c3))))
    ...
    private fun solveNxN(aMatrix: Array<DoubleArray>, bVector: DoubleArray): LinearSystemResult {
        // Fallback robust N x N implementation
        val n = bVector.size
        val steps = mutableListOf<LinearStep>()
        val a = Array(n) { r -> aMatrix[r].copyOf() }
        val b = bVector.copyOf()
        val p = IntArray(n) { it }
        val l = Array(n) { DoubleArray(n) { j -> if (n == j) 1.0 else 0.0 } }
        ...
        try {
            for (i in 0 until n) {
                ...
                if (abs(a[i][i]) < 1e-12) throw SingularMatrixException("Zero pivot in LU")
    ...
    private fun copyMatrix(a: Array<DoubleArray>): Array<DoubleArray> = Array(a.size) { r -> a[r].copyOf() }
}
