...
import com.numerical.analysis.solver.domain.solvers.*
import kotlinx.coroutines.flow.*
import com.numerical.analysis.solver.domain.MathParser
import com.numerical.analysis.solver.domain.methods.GoldenSectionSolver
...
import com.numerical.analysis.solver.domain.methods.LinearAlgebraMethods
...
class SolverViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun updateRootFindingInput(
        ...
        toleranceMode: com.numerical.analysis.solver.ui.state.ToleranceMode? = null
    ) {
    ...
    fun solveRootPath(method: String) {
        ...
        calculationJob = viewModelScope.launch {
            ...
            try {
                ...
                val (step1, conv, res) = withContext(Dispatchers.Default) {
                    ...
                    when (method) {
                        "Bisection" -> {
                            ...
                            val results = BisectionSolver.solve(xl, xu, eps, maxIter, state.toleranceMode, f)
                        ...
                        "False Position" -> {
                            val xl = state.xl.toDouble()
                            val xu = state.xu.toDouble()
                            val results = FalsePositionSolver.solve(xl, xu, eps, maxIter, state.toleranceMode, f)
                            if (results.isEmpty()) throw Exception("Zero iterations computed.")
                            Triple(results, results.last().error <= eps, results.lastOrNull()?.xr)
                        }
                        "Newton" -> {
                            ...
                            val results = NewtonSolver.solve(xi, eps, maxIter, state.toleranceMode, f, fDash)
                            if (results.isEmpty()) throw Exception("Zero iterations computed.")
                            Triple(results, results.last().error <= eps, results.lastOrNull()?.xiPlus1)
                        }
                        "Fixed Point" -> {
                            val xi = state.xi.toDouble()
                            val results = FixedPointSolver.solve(xi, eps, maxIter, state.toleranceMode, f)
                            if (results.isEmpty()) throw Exception("Zero iterations computed.")
                            Triple(results, results.last().error <= eps, results.lastOrNull()?.xiPlus1)
                        }
                        "Secant" -> {
                ...
                val isPercentage = state.toleranceMode == com.numerical.analysis.solver.ui.state.ToleranceMode.PERCENTAGE
                ...
                _rootFindingState.update {
                    if (method in listOf("Bisection", "False Position")) {
                        ...
                        val finalized = it.copy(
                            bracketingResults = step1 as List<com.numerical.analysis.solver.domain.methods.BracketingStep>,
